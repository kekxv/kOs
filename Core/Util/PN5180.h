...
extern struct PN5180 pn5180;

struct PN5180 {
    uint8_t PN5180_NSS[2]; // active low
    uint8_t PN5180_BUSY[2];
    uint8_t PN5180_RST[2];
    uint8_t readBuffer[508];
    int8_t (*digitalRead)(uint8_t pin[2]);
    void (*digitalWrite)(uint8_t pin[2], GPIO_PIN_STATE state);

    void (*PN5180)(uint8_t SSpin[2], uint8_t BUSYpin[2], uint8_t RSTpin[2]);
   ...
   */
    ...
    /* cmd 0x09 */
    bool (*sendData)(uint8_t *data, int len, uint8_t validBits);
    /* cmd 0x0a */
   ...
   */
    bool (*transceiveCommand)(uint8_t *sendBuffer, size_t sendBufferLen, uint8_t *recvBuffer, size_t recvBufferLen);
};
void PN5180_Init();
